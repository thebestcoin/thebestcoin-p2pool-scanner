...
function dpc(t, fn) {
    if (typeof(t) == 'function') setTimeout(t, 0); else setTimeout(fn, t);
}
...
function Scanner(options) {
    ...
    self.render = function () {
        var str = "<html><head>"
            ...
            + ".p2p {  width: 888px; margin: auto; border: 1px solid #aaa;  box-shadow: 2px 2px 2px #aaa; padding: 2px;  }"
            + ".p2p-row { width: 870px; padding: 10px; height: 16px; }"
            + ".p2p-caption { width: 870px; text-align: center;  background-color: #ddd; padding-top: 4px; padding-bottom: 8px;}"
            + ".p2p div { float : left; }"
            + ".p2p-ip { width: 250px; text-align:left; }"
            ...
            + "</head><body>";
        if (logo)
            str += "<div style='text-align:center;'><img width=\"100\" src=\"" + logo + "\" /></div><br style='clear:both;'/>";
        str += "<center><a href='" + (config.currency_url) + "' target='_blank'>PEER TO PEER " + (config.currency.toUpperCase()) + " MINING NETWORK</a> - PUBLIC NODE LIST<br/><span style='font-size:10px;color:#333;'>GENERATED ON: " + (new Date()) + "</span></center><p/>"
        if (self.poolstats)
        ...
        var list = _.sortBy(_.toArray(self.addr_working), function (o) {
            return o.stats ? -o.stats.uptime : 0;
        })
        ...
        _.each(list, function (info) {
            ...
            var host = nodes[ip] || info.ip + '38802';

            var version = info.stats.version;
            var uptime = (info.stats.uptime / 60 / 60 / 24).toFixed(1);
            var fee = (info.fee || 0).toFixed(2);

            str += "<div class='p2p-row " + (row++ & 1 ? "row-grey" : "") + "'><div class='p2p-ip'><a href='http://" + host + "/static/' target='_blank'>" + host + "</a></div><div class='p2p-version'>" + version + "</div><div class='p2p-fee'>" + fee + "%</div><div class='p2p-uptime'>" + uptime + " days</div>";
    ...
    self.digest = function () {
        ...
        var info = _.find(self.addr_pending, function () {
            return true;
        });
        ...
        console.log("P2POOL DIGESTING:" + info.ip + ":" + 38802);

        var allowedVersions = ["f503e31-dirty", "dd1753b-dirty", "5387358-dirty", "07b1092"];
            ...
            if (!err) {
                ...
                digest_local_stats(info, function (err, stats) {
                    if (!err && allowedVersions.indexOf(stats.version) >= 0) {
                        info.fee = fee;
                        info.stats = stats;
                        console.log("FOUND WORKING POOL: " + info.ip + ":38802 " + info.stats.version);
